...
public class MainActivity extends FlutterActivity {
    ...
    private boolean isTransactionMessage(String sender, String body) {
        if (body == null || sender == null) return false;

        String lowerBody = body.toLowerCase();
        sender = sender.toLowerCase();

        String[] keywords = {"debited", "txn", "transaction", "spent", "rs", "inr", "balance", "amount", "upi", "payment", "purchased","loan",
        "emi"};
        String[] banks = {
            ...
            if (bankMatch) {
                for (String keyword : keywords) {
                    if (lowerBody.contains(keyword)) {
                        if (lowerBody.matches(".*\\bxx[0-9]{3,4}\\b.*") || lowerBody.matches(".*\\bending in [0-9]{4}\\b.*")) {
